...
import { Download, Loader2, CheckCircle2, XCircle, Clock, Video as VideoIcon } from 'lucide-react';
...
function TaskItem({ task, onRefresh }: TaskItemProps) {
  ...
  const isTaskDownloading = task.status === 'downloading';
        ...
        <div className="task-actions" style={{ marginLeft: '1rem' }}>
          ...
          {task.status !== 'searching' && !isAllCompleted && (
            <button 
              ...
              disabled={triggeringTask || isTaskDownloading}
            >
              {triggeringTask || isTaskDownloading ? <Loader2 className="spin" size={16} /> : <Download size={16} />}
              {isTaskDownloading ? '批量下载中...' : '一键下载全部'}
            </button>
          )}
        </div>
      </div>
